...
#### Ejercicio 2 Encuentra una manera de identificar la pareja de variables que tienen el valor máximo de correlación utilizando código de R {.unnumbered}

```{r eval=FALSE, include=FALSE}
cormat <- cor(mtcars[,c("disp", "hp", "wt", "drat")])
cormat[cormat == 1] <- 0
which(cormat==max(cormat), arr.ind=TRUE)
```
...
## Selección de modelos en regresión lineal múltiple
