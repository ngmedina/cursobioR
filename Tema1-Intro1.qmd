...
Vamos a trabajar con los datos de una base de datos de la abundancia de especies en herbazales de Alberta en Canada.
...
### [6.4 EXTRA: Importar datos desde Excell]{style="color:darkblue"}
...
```{webr-r, error=TRUE}
datos <- read.csv("misdatos/datos_con_errores.csv", sep = ";")
datos
```
...
list.files()  # listar archivos disponibles
```
...
datos <- read.csv("data/datos_con_errores.csv", sep = ";")
datos
```
...
```{webr-r}
datos <- read.csv("data/datos_con_errores.csv", sep = ";", na.strings = c("", "NA", ".", "?"))
datos
```
