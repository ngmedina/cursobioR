...
Todo el código de R está disponible como software [libre](https://es.wikipedia.org/wiki/Software_libre) bajo las condiciones de [licencia GNU-GPL](https://es.wikipedia.org/wiki/GNU_General_Public_License). Esto implica que cualquier usuario final puede **usar, estudiar, compartir (copiar) y modificar el software**.
...
#### Ejercicio 9: Cargar datos integrados
...
```{webr-r}
datos <- read.csv("https://github.com/ngmedina/cursobioR/blob/main/data/datos_con_errores.csv")
View(datos)
```
...
```{webr-r}
datos <- read.csv("https://github.com/ngmedina/cursobioR/blob/main/data/datos_con_errores.csv", fileEncoding = "latin1")
View(datos)
```
...
```{webr-r}
datos <- read.csv("https://github.com/ngmedina/cursobioR/blob/main/data/datos_con_errores.csv", fileEncoding = "latin1", sep = ",")
View(datos)
```
