...
```{r}
# read.csv("C:/Users/TuNombre/datos_con_errores.csv") # Tú código debería ser algo así
read.csv("data/datos_con_errores.csv")
```
...
```{r}
datos <- read.csv("data/datos_con_errores.csv")
View(datos)
```
...
```{webr-r}
datos <- read.csv("data/datos_con_errores.csv", fileEncoding = "latin1")
View(datos)
```
...
```{webr-r}
datos <- read.csv("data/datos_con_errores.csv", fileEncoding = "latin1", sep = ",")
View(datos)
```
...
¿Edad es numérica? ¿Fecha es tipo Date?
